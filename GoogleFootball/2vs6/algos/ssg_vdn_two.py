...
class SSG_VDN():
    ...
    def _get_critic_inputs(self, s, ss, a1):
        ...
        # 添加所有agent的上一个动作
        u_onehot_last = u_onehot_last.reshape(self.arg_dict['rollout_len'], self.arg_dict['batch_size'], 1, -1).repeat(1, 1,self.n_agents, 1)
        u_onehot_ = u_onehot.reshape(self.arg_dict['rollout_len'], self.arg_dict['batch_size'], 1, -1).repeat(1, 1,self.n_agents, 1)
        ...
        '''
        u_onehot_leader_0 = torch.cat((torch.zeros(*u_onehot[:, :, [0], :].shape), u_onehot[:, :, [1], :]), dim=2)
        u_onehot_next_leader_0 = torch.cat((torch.zeros(*u_onehot[:, :, [0], :].shape), u_onehot_next[:, :, [1], :]), dim=2)
        u_onehot_follower_0 = torch.cat((u_onehot[:, :, [0], :], torch.zeros(*u_onehot[:, :, [1], :].shape)), dim=2)
        u_onehot_next_follower_0 = torch.cat((u_onehot_next[:, :, [0], :], torch.zeros(*u_onehot[:, :, [1], :].shape)), dim=2)

        u_onehot_0 = torch.cat((u_onehot_leader_0, u_onehot_follower_0), dim=-1)
        u_onehot_next_0 = torch.cat((u_onehot_next_leader_0, u_onehot_next_follower_0), dim=-1)
        ...
        '''
        inputs.append(torch.eye(self.n_agents).unsqueeze(0).unsqueeze(0).expand(self.arg_dict['rollout_len'], self.arg_dict['batch_size'], -1, -1))
        inputs_next.append(torch.eye(self.n_agents).unsqueeze(0).unsqueeze(0).expand(self.arg_dict['rollout_len'], self.arg_dict['batch_size'], -1, -1))
    ...
    def td_lambda_target(self, q_targets, r, mask, terminated):
        ...
        terminated = (1-terminated).squeeze(-1).permute(2,1,0) # rjq terminated error
        ...
        for transition_idx in range(max_episode_len - 1, -1, -1):
            ...
            n_step_return[:, transition_idx, :, 0] = (r[:, transition_idx] + self.gamma *\
                                                      q_targets[:, transition_idx] * terminated[ :, transition_idx]) * mask[ :, transition_idx]  # 经验transition_idx上的obs有max_episode_len - transition_idx个return, 分别计算每种step return
            ...
            for n in range(1, max_episode_len - transition_idx):
                ...
                n_step_return[:, transition_idx, :, n] = (r[:, transition_idx] + self.gamma *\
                                                          n_step_return[:, transition_idx + 1, :, n - 1]) * mask[:, transition_idx]
        # --------------------------------------------------n_step_return---------------------------------------------------
    ...
    def train(self, model, follower_model, mixer, coma_critic, data):
        ...
        for mini_batch in data:
            ...
            s = torch.cat((s[ind].unsqueeze(0), s[ind+1].unsqueeze(0)), dim=0) # 状态的存储是根据leader\follower存储的，每隔一个取一个
            ...
            a1 = torch.cat((a1[ind].unsqueeze(0), a1[ind + 1].unsqueeze(0)), dim=0)
            avail_u = torch.cat((avail_u[ind].unsqueeze(0), avail_u[ind + 1].unsqueeze(0)), dim=0)
            mask = torch.cat((done_mask[ind].unsqueeze(0), done_mask[ind+1].unsqueeze(0)), dim=0)[0]  # 10.4.1 ; leader和follower的done_mask是一致的
            ...
            follower_q , _= follower_model(s[1], leader_q, h_in[1])
            ...
            q_values_bk = torch.stack((leader_q, follower_q), dim=0)  # num_agents \ rollout_len\ batch_size\ action_dim
            ...
            q_targets_bk = torch.stack((target_q_, target_follower_q_), dim=0)  # num_agents \ rollout_len\ batch_size\ action_dim
            ...
            done_mask_2 = torch.cat((done_mask[ind].unsqueeze(0), done_mask[ind+1].unsqueeze(0)), dim=0) # torch.Size([2, 10, 4, 1]) ### 此处指的是done_mask
            ...
            if not self.arg_dict['w_vdn']:
                ...
                ws = torch.ones_like(td_error.permute(1,0,2)) * w_to_use  # (1.10.4)
            ...
            q_evals = torch.gather(q_eval, dim=3, index=a).squeeze(3)
