...
def actor(actor_num, center_model, center_follower_model, data_queue, signal_queue, summary_queue, arg_dict):
    ...
    while True:  # episode loop
        ...
        while not done and steps < arg_dict["episode_limit"]:  # step loop
            ...
            leader_id = 0
            if arg_dict['algorithm'] == 'ssg_vdn' or arg_dict['algorithm'] == 'ssg_vdn_two':
                ...
                actions_, h_out, leader_id = choose_action_asg(obs_model, last_action, leader_id, steps,
                                                             avail_action_s, epsilon, arg_dict, model, follower_model, h_in)
            ...
            if done[0]:
                ...
                if arg_dict['algorithm'] == 'ssg_vdn' or arg_dict['algorithm'] == 'ssg_vdn_two':
                    ...
                    #                               avail_action_s, epsilon, arg_dict, model, follower_model, h_in)
                    actions_, h_out, leader_id = choose_action_asg(obs_model, last_action, leader_id, steps,
                                                                   avail_action_s, epsilon, arg_dict, model,
                                                                   follower_model, h_in)
                    for agent_id in range(arg_dict["n_agents"]):
                        action = actions_[agent_id]
                        action_onehot = np.zeros(arg_dict["n_actions"])
