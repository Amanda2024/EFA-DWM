...
def learner(center_model, center_follower_model, center_mixer, center_coma_critic, queue, data_all_queue, signal_queue, summary_queue, arg_dict):
    print("Learner process started")
    imported_model = importlib.import_module("models.agents." + arg_dict["model"])
    imported_follower_model = importlib.import_module("models.agents." + arg_dict["follower_model"])
    ...
    follower_model = imported_follower_model.Follower_Agent(input_shape, arg_dict, device)
    ...
    imported_coma_critic = importlib.import_module("models.agents." + arg_dict["coma_critic"])
    coma_critic = imported_coma_critic.ComaCritic(arg_dict['critic_input_shape'], arg_dict)
    coma_critic.load_state_dict(center_coma_critic.state_dict())

    algo = imported_algo.SSG_VDN(arg_dict, model, follower_model, mixer, coma_critic)
    ...
    while True:
        if queue.qsize() > arg_dict["batch_size"] * arg_dict["buffer_size"] * arg_dict["rich_data_scale"]:
            ...
            print("data loaded……")
            loss = algo.train(model, follower_model, mixer, coma_critic, data)
            ...
            optimization_step += 1
            print("step :", optimization_step, "loss", loss, "data_q", queue.qsize(), "summary_q",
                  summary_queue.qsize())

            loss = loss.cpu().detach().numpy()
            loss_lst.append(loss)
            center_model.load_state_dict(model.state_dict())  # How to load the q-agent and mixer together
            center_follower_model.load_state_dict(follower_model.state_dict())
            center_mixer.load_state_dict(mixer.state_dict())
            center_coma_critic.load_state_dict(coma_critic.state_dict())
