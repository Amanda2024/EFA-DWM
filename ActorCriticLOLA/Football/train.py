...
def main(arg_dict):
    for i in range(0, 5, 1):
        ...
        center_model_1 = model.RNNAgent(input_shape, arg_dict)
        center_model_2 = model.RNNAgent(input_shape, arg_dict)
        ...
        model_dict = {
            ...
            'model_state_dict1': center_model_1.state_dict(),
            'optimizer_state_dict1': center_model_1.optimizer.state_dict(),
            'model_state_dict2': center_model_2.state_dict(),
            'optimizer_state_dict2': center_model_2.optimizer.state_dict(),
        }
        ...
        center_model_1.share_memory() #进程间通信
        center_model_2.share_memory() #进程间通信
        ...
        p = mp.Process(target=learner, args=(center_model_1, center_model_2, center_mixer, data_queue, data_all_queue, signal_queue, summary_queue, arg_dict))  # target：调用对象/目标函数  args：调用对象的位置参数元组/按位置给目标函数传参
        p.start()
        processes.append(p)
        ...
        for rank in range(arg_dict["num_processes"]):
            ...
            if arg_dict["env"] == "11_vs_11_kaggle":
                ...
                p = mp.Process(target=actor, args=(rank, center_model_1, center_model_2, data_queue, signal_queue, summary_queue, arg_dict))
            p.start()
            processes.append(p)
...
if __name__ == '__main__':
    ...
    arg_dict = {
        ...
        "env_evaluation":'academy_run_to_score_with_keeper',  # for evaluation of self-play trained agent (like validation set in Supervised Learning),
