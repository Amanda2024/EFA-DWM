...
class QMixNet(nn.Module):
    ...
    def forward(self, q_values, states):  # states的shape为(episode_num, max_episode_len， state_shape)
        ...
        q_values = q_values.view(-1, 1, self.args.n_agents)  # (episode_num * max_episode_len, 1, n_agents) = (1920,1,5) (50,1,2)
        ...
        w1 = w1.view(-1, self.args.n_agents, self.args.qmix_hidden_dim)  # (50, 2, 32)
        b1 = b1.view(-1, 1, self.args.qmix_hidden_dim)  # (50, 1, 32)

        hidden = F.elu(torch.bmm(q_values, w1) + b1)  # (1920, 1, 32)

        w2 = torch.abs(self.hyper_w2(states))  # (50, 32)
        b2 = self.hyper_b2(states)  # (1920, 1)

        w2 = w2.view(-1, self.args.qmix_hidden_dim, 1)  # (50, 32, 1)
        b2 = b2.view(-1, 1, 1)  # (1920, 1， 1)

        q_total = torch.bmm(hidden, w2) + b2  # (50, 1, 1)
        q_total = q_total.view(episode_num, -1, 1)  # (1, 50, 1)
        return q_total
