...
class ReplayBuffer:
    def __init__(self, args):
        ...
        self.buffers = {'o': np.empty([self.size, self.episode_limit, self.n_agents, self.obs_shape]),
                        ...
                        's': np.empty([self.size, self.episode_limit, self.n_agents, self.state_shape]),
                        'r': np.empty([self.size, self.episode_limit, 1, self.n_agents]),
                        'o_next': np.empty([self.size, self.episode_limit, self.n_agents, self.obs_shape]),
                        's_next': np.empty([self.size, self.episode_limit, self.n_agents, self.state_shape]),
                        ...
                        'lead_ids': np.empty([self.size, self.episode_limit, 1])
                        }
    ...
    def store_episode(self, episode_batch):
        ...
        with self.lock:
            ...
            self.buffers['lead_ids'][idxs] = episode_batch['lead_ids']
            if self.args.alg == 'maven':
