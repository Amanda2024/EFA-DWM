...
class SSG_VDN_TWO:
    def __init__(self, args):
        ...
        self.eval_parameters = list(self.eval_vdn_net.parameters()) + list(self.eval_rnn.parameters()) + list(
            self.eval_follower.parameters())
        ...
        print('Init alg SSG-VDN-Two_Step')
    ...
    def learn(self, batch, max_episode_len, train_step, epsilon=None):  # train_step表示是第几次学习，用来控制更新target_net网络的参数
        ...
        s, s_next, u, r, avail_u, avail_u_next, terminated = batch['s'], batch['s_next'], batch['u'], \
                                                             ...
                                                             batch['terminated']
        terminated_next = torch.cat((terminated[:, 1:, :], terminated[:, -1, :].unsqueeze(1)), dim=1)
        avail_u_next_next = torch.cat((avail_u_next[:, 1:, :, :], avail_u_next[:, -1, :, :].unsqueeze(1)), dim=1)
        mask = 1 - batch["padded"].float()  # 用来把那些填充的经验的TD-error置0，从而不让它们影响到学习

        # 得到每个agent对应的Q值，维度为(episode个数, max_episode_len， n_agents， n_actions)
        q_evals, q_targets = self.get_q_values(batch, max_episode_len)  # Q(s,a) and Q(s'', a'')
        if self.args.cuda:
            s = s.cuda()
            u = u.cuda()
            r = r.cuda()
        ...
        # 得到target_q
        q_targets[avail_u_next_next == 0.0] = - 9999999
        ...
        r_next = torch.cat((r[:, 1:, :, :], r[:, -1, :, :].unsqueeze(1)), dim=1)
        r = 0.5 * torch.sum(r, dim=-1).reshape(q_total_eval.shape[0], -1, 1)  # Total reward for each agent
        r_next = 0.5 * torch.sum(r_next, dim=-1).reshape(q_total_eval.shape[0], -1, 1)  # Total reward for each agent
        targets = r + self.args.gamma * (r_next + (self.args.gamma * q_total_target * (1 - terminated_next)))
    ...
    def _get_inputs(self, batch, transition_idx):
        ...
        if self.args.alg == 'ssg_vdn_two':
            obs_next_next = batch['o_next']
            obs_next_next = torch.cat((obs_next_next[:, 1:, :, :], obs_next_next[:, -1, :, :].unsqueeze(1)), dim=1)[:,
                            transition_idx]

        episode_num = obs.shape[0]
        inputs, inputs_next_next = [], []
        inputs.append(obs)
        inputs_next_next.append(obs_next_next)
        # 给obs添加上一个动作、agent编号
        ...
        if self.args.last_action:
            ...
            if transition_idx < u_onehot.shape[1] - 1:
                inputs_next_next.append(u_onehot[:, transition_idx + 1])
            else:
                inputs_next_next.append(u_onehot[:, transition_idx])  # 最后一个时刻输入的T+1动作应为最后一个时刻的动作
        ...
        if self.args.reuse_network:
            ...
            inputs_next_next.append(torch.eye(self.args.n_agents).unsqueeze(0).expand(episode_num, -1, -1))
        # 要把obs中的三个拼起来，并且要把episode_num个episode、self.args.n_agents个agent的数据拼成40条(40,96)的数据，
        # 因为这里所有agent共享一个神经网络，每条数据中带上了自己的编号，所以还是自己的数据
        # inputs = torch.cat([x.reshape(episode_num * self.args.n_agents, -1) for x in inputs], dim=1)
        inputs = torch.cat([x.reshape(episode_num, self.args.n_agents, -1) for x in inputs], dim=-1)
        # inputs_next = torch.cat([x.reshape(episode_num * self.args.n_agents, -1) for x in inputs_next], dim=1)
        inputs_next_next = torch.cat([x.reshape(episode_num, self.args.n_agents, -1) for x in inputs_next_next], dim=-1)
        return inputs, inputs_next_next
    ...
    def get_q_values(self, batch, max_episode_len):  # New for ssg with asymmetric structure
        ...
        for transition_idx in range(max_episode_len):
            inputs, inputs_next = self._get_inputs(batch,
                                                   transition_idx)  # 给obs加last_action、agent_id，此时第二个输入为s^{''}传给target网络，目的是two-step update
            if self.args.cuda:
                inputs = inputs.cuda()
            ...
            q_eval, self.eval_hidden = self.eval_rnn(inputs[:, 0, :],
                                                     self.eval_hidden)  # inputs维度为(40,96)，得到的q_eval维度为(40,n_actions) for the leader
            q_eval_f, self.eval_follower_hidden = self.eval_follower(inputs[:, 1, :], q_eval, self.eval_follower_hidden)

            q_target, self.target_hidden = self.target_rnn(inputs_next[:, 0, :], self.target_hidden)
            q_target_f, self.target_follower_hidden = self.target_follower(inputs_next[:, 1, :], q_target,
                                                                           self.target_follower_hidden)

            # concat along the dimension of num_agents
            q_eval = torch.stack((q_eval, q_eval_f), dim=1)
            q_target = torch.stack((q_target, q_target_f), dim=1)
